...
// 		console.log(resvNum);
		location.href=("detailMyResv.jsp?resvNum="+resvNum);
	});
});
...
<ul class="pagination">
	...
	if(startPage!=1){
		%>
			<li>
				<a href="?resvType=<%=resvType %>&pageNum=<%=startPage-1%>">이전</a>
			</li>
		<%
	}
	for(int i=startPage;i<=endPage;i++){
		...
		if(i==currentPage){%>
			<li class="active">
				<a href="?resvType=<%=resvType %>&pageNum=<%=i%>"><%=i %></a>
			</li>
		<%}else{ %>
			<li>
				<a href="?resvType=<%=resvType %>&pageNum=<%=i%>"><%=i %></a>
		...
		%>
			<li>
				<a href="?resvType=<%=resvType %>&pageNum=<%=endPage+1%>">다음</a>
			</li>
		<%
	}
		%>
